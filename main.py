...
class WheelCanvas(tk.Canvas):
    ...
    def __init__(self, master: tk.Widget) -> None:
        ...
        self.user_slots: Dict[str, list[str]] = {}
        self.current_phrase = ""
    ...
    def set_user_slots(self, user_slots: Dict[str, list[str]]) -> None:
    ...
    def draw_wheel(self) -> None:
        ...
        if total_votes <= 0:
            ...
            for idx, (phrase, votes) in enumerate(self.entries.items()):
                ...
                slots = self.user_slots.get(phrase, [])
                if len(slots) > 1:
                    slot_extent = extent / len(slots)
                    for i in range(1, len(slots)):
                        boundary_angle = math.radians(start_angle + i * slot_extent)
                        x2 = cx + math.cos(boundary_angle) * radius
                        y2 = cy - math.sin(boundary_angle) * radius
                        self.create_line(cx, cy, x2, y2, fill="#ffffff", width=1)
...
class App:
    ...
    def build_phrase_user_slots(self, top_votes: Dict[str, int]) -> Dict[str, list[str]]:
        ...
        for phrase, votes in top_votes.items():
            ...
            phrase_slots[phrase] = voter_slots
        return phrase_slots
    ...
    def pointer_details(self) -> Tuple[str, str]:
        ...
        phrase_slots = self.build_phrase_user_slots(top_votes)
        pointer_angle = 90.0
        ...
        for phrase, votes in top_votes.items():
            ...
            if running <= wheel_angle < running + extent:
                voter_slots = phrase_slots.get(phrase, [])
                if not voter_slots:
    ...
    def refresh_table_from_votes(self) -> None:
        ...
        self.wheel_canvas.set_user_slots(self.build_phrase_user_slots(top_votes))
